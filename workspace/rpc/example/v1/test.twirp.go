...
/*
Package example_v1 is a generated twirp stub package.
...
func (c *echoProtobufClient) SayHello(ctx context.Context, in *HelloRequest) (*HelloResponse, error) {
	ctx = ctxsetters.WithPackageName(ctx, "example.v1")
...
func (c *echoJSONClient) SayHello(ctx context.Context, in *HelloRequest) (*HelloResponse, error) {
	ctx = ctxsetters.WithPackageName(ctx, "example.v1")
	ctx = ctxsetters.WithServiceName(ctx, "Echo")
	ctx = ctxsetters.WithMethodName(ctx, "SayHello")
	out := new(HelloResponse)
...
const EchoPathPrefix = "/twirp/example.v1.Echo/"
...
func (s *echoServer) ServeHTTP(resp http.ResponseWriter, req *http.Request) {
	...
	switch req.URL.Path {
	case "/twirp/example.v1.Echo/SayHello":
...
func (s *echoServer) serveSayHello(ctx context.Context, resp http.ResponseWriter, req *http.Request) {
	...
	case "application/x-www-form-urlencoded", "multipart/form-data":
		s.serveSayHelloForm(ctx, resp, req)
	default:
		if req.Method == "GET" {
			s.serveSayHelloForm(ctx, resp, req)
			return
...
func (s *echoServer) serveSayHelloJSON(ctx context.Context, resp http.ResponseWriter, req *http.Request) {
	var err error
	ctx = ctxsetters.WithMethodName(ctx, "SayHello")
	...
	func() {
		...
		respContent, err = s.Echo.SayHello(ctx, reqContent)
	}()
	...
	if respContent == nil {
		s.writeError(ctx, resp, twirp.InternalError("received a nil *HelloResponse and nil error while calling SayHello. nil responses are not supported"))
		return
	}
	...
	if body, ok := interface{}(respContent).(httpBody); ok {
		...
		resp.Header().Set("Content-Type", body.GetContentType())
	} else {
...
func (s *echoServer) serveSayHelloProtobuf(ctx context.Context, resp http.ResponseWriter, req *http.Request) {
	var err error
	ctx = ctxsetters.WithMethodName(ctx, "SayHello")
	ctx, err = callRequestRouted(ctx, s.hooks)
	if err != nil {
...
func (s *echoServer) serveSayHelloForm(ctx context.Context, resp http.ResponseWriter, req *http.Request) {
	var err error
	ctx = ctxsetters.WithMethodName(ctx, "SayHello")
	ctx, err = callRequestRouted(ctx, s.hooks)
	if err != nil {
	...
	func() {
		...
		respContent, err = s.SayHello(ctx, reqContent)
	}()
